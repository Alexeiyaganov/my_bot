...
class Org:
    ...
    def question2(self, message, db, date):
        ...
        m2 = self.bot.send_message(message.chat.id,
                                   text="Загрузите  информацию о мероприятии(документ(pdf, zip) или картика",
                                   reply_markup=markup)
    ...
    def question3(self, message, db, date):
        if message.text == "Не выкладывать информацию":
            self.LINKS.append("")
        elif message.document == None and message.photo == None:
        ...
        m4 = self.bot.send_message(message.chat.id,
                                   text="Разместить заявку тут или прикрепить ссылку на внешний источник?",
                                   reply_markup=markup)
    ...
    def question4(self, message, db, date):
        if message.text == "Открыть заявку тут":
            self.LINKS.append("")
            self.LINKS.append(1)
            items = db.get_groups_by_phone(self.LINKS[0])
    ...
    def final_push(self, message, db, date, items=None):
        check_here = self.LINKS.pop()
        checkurl = self.LINKS.pop()
        info = self.LINKS.pop()
        event_name = self.LINKS.pop()
        phone = self.LINKS.pop()
        event_id = db.add_event(org_phone=phone, event_name=event_name, event_date=date, info=info, checkurl=checkurl,
                                check_here=check_here, location="", split="")
        if items is not None:

            db.create_groups_event_table(event_id)
            db.add_groups_to_event(event_id, items)
            markup = self.bc.make_user_buttons()
            self.bot.send_message(message.chat.id, text="Поздравляем! Информация о мероприятии добавлена",
                                  reply_markup=markup)
