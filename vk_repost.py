        ...
        if 'copy_history' in post:
            ...
            if 'attachments' in copy_history:
                ...
                if copy_add['type'] == 'link':
                    link = copy_add['link']
                    url = link['url']
                    text_link = link['title']
                    # send_posts_text(text_link)
                    img = link['photo']
                    send_posts_img(img, link + "\n" + text_link)
                    # url = link['url']
                    # send_posts_text(url)
                ...
                if copy_add['type'] == 'photo':
                    ...
                    for add in attach:
                        try:
                            ...
                            print("Не получилось открыть фото")
                    ...
                    if len(images) > 0:
                        ...
                        print(image_urls)
                        media_group = []
                        for num, item in enumerate(image_urls):
                            media_group.append(InputMediaPhoto(item,
                                                               caption=text if num == len(image_urls)-1 else ''))
                        bot.send_media_group(CHANNEL, media = media_group)
